...
**Provenance:** started 2026-09-24 as a copy of `harbor-on-kind` @ `b65df71` (full git history kept; not a GitHub fork, since GitHub forbids same-owner forks). Original single-node lab: https://github.com/it255ru/harbor-on-kind. Everything under "Baseline" below is verified-working *single-node* behavior inherited from there. **HA work status:** Phases 0–4 done and re-verified on a stand rebuilt from scratch (14-node cluster, Infra LB, Consul, Patroni/PostgreSQL, Valkey/Sentinel, HAProxy, Garage S3, Harbor in HA; milestone 1 accepted by the user 2026-09-24; milestone 2 tests H4.1–H4.7 and the from-scratch acceptance H5.3 passed). Left: H5.2 final review of AGENTS.md/CLAUDE.md, H5.4 Ansible port, H5.5 image cache (needs the user's decision) — the plan and decisions are in `backlog.md` (written in Russian, source of truth).
...
- **Success is two-staged (D6):** first *milestone 1* — the whole stand works with correct distribution over the 14 nodes (roles, placement, healthy Consul/Patroni/Redis, end-to-end push/pull via the full chain, both app replicas serving); only after it passes do the Phase 4 failure tests (*milestone 2*) count. Milestone 1 (`H3.5`) is accepted (2026-09-24), Phase 4 (milestone 2) passed and was re-run on a rebuilt stand.
...
- Redis safety (H4.7): `start-valkey.sh` must not make a restarted `redis-0` master before it has asked the peers (fresh volume vs restart), Valkey runs with `min-replicas-to-write 1`, the HAProxy Redis check requires `role:master` + a connected replica (regex `role:master[^a-z]{1,4}connected_slaves:[1-9]`; in HAProxy regexes `.` does NOT match a newline). Harbor core/jobservice liveness probes are relaxed (timeout 5 s, 6 failures) because they hang while Redis is unreachable.
- Failure-test scripts (`hack/tests/h4*.sh`) kill or throttle real nodes: `h47-role-failure.sh` always starts the victim node again on exit; if it is interrupted, run `docker start <node>` and check `kubectl get nodes`.
